...
/*   Updated: 2025/08/31 23:16:57 by jmiguele         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_parse_nbr(char *nbr)
{
	...
	{
		ft_putstr("ERROR PARSE NBR 23", 2);
		exit (1);
	}
	i = 0;
	while (nbr[i])
...
char	**help_triades(char **mtx, int lenght)
{
	int		j;
	int		i;
	...
	while (i < (lenght / 3 + 1))
	{
		...
		if (mtx == NULL)
		{
			ft_putstr("Error parse_nbr. linea 64", 2);
			exit(1);
		}
		while (j < 3)
		{
...
char	**make_triades(int length)
{
	...
	if (mtx == NULL)
	{
		ft_putstr("ERROR AL ASIGNAR MEMORIA\n", 2);
		exit(1);
	}
	while (i < (length / 3 + 1))
	{
